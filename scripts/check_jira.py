...
def do_request(url, *args, **kwargs):
    ...
    response = requests.get(url, *args, timeout=30, **kwargs)
...
def jira_apis():
    ...
    Path(".guardette/jira_issues_response.json").write_text(json.dumps(response, indent=4))

    # List Jira boards using REST 1.0 API
    boards_response = do_request(f"{proxy_base_url}/rest/agile/1.0/board", headers=headers)

    Path(".guardette/jira_boards_response.json").write_text(json.dumps(boards_response, indent=4))
    ...
    Path(".guardette/jira_board_issues_response.json").write_text(json.dumps(issues_response, indent=4))
